...
def gns_search(
    ...
    try:
        ...
        newargs["startdate"] = starttime.strftime(TIMEFMT) #f'{date.fromisoformat(starttime).strftime(TIMEFMT)}'
        newargs["enddate"] = endtime.strftime(TIMEFMT) #f'{date.fromisoformat(endtime).strftime(TIMEFMT)}'
